...
def check_security_groups(ec2_client, tag_key, tag_values):
    ...
    # Define restricted management and database ports
    restricted_ports = {
        22: "SSH",
        3389: "RDP",
        5432: "PostgreSQL",
    ...
    try:
        # Fetch security groups matching our tags
        ...
        for sg in sgs:
            ...
            for rule in sg.get('IpPermissions', []):
                ...
                # Check for public/unrestricted ingress on sensitive ports
                for ip_range in rule.get('IpRanges', []):
                    cidr = ip_range.get('CidrIp')
                    if cidr == '0.0.0.0/0':
                        # Check if it overlaps with restricted ports or allows all ports
...
def check_s3_buckets(s3_client, tag_key, tag_values):
    ...
    try:
        ...
        for bucket in buckets:
            ...
            # Retrieve bucket tags to check tag alignment
            try:
                tag_response = s3_client.get_bucket_tagging(Bucket=bucket_name)
                tags = {t['Key']: t['Value'] for t in tag_response.get('TagSet', [])}
            except ClientError as e:
                # If no tags exist or access denied, skip tag matching and filter by name prefix as fallback
            ...
            # Filter to check only buckets aligned with our tags
