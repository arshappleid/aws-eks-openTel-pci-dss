module "alb" {
  ...
  listeners = {
    http-default = {
      port     = 80
      protocol = "HTTP"
      # Default fallback forwards to the default Target Group (could also be a 404 fixed-response)
      forward = {
...
# Dynamically generate ALB path-based routing rules based on the var.services map
